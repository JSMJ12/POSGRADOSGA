...
<div class="container">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <table class="table" id="secretarios">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Foto</th>
                ...
                <tbody>
                    @foreach ($secretarios as $secretario)
                        <tr>
                            ...
                            <td>
                                {{ $secretario->seccion ? $secretario->seccion->nombre : 'No asignado' }}<br>
                                    Maestrías:
                                        @if ($secretario->seccion)
                                            @foreach ($secretario->seccion->maestrias as $maestria)
                                            <li>{{ $maestria->nombre }}<br></li>
                                            @endforeach
                                        @else
                                            No asignada
                                        @endif
                        ...
                        <!-- Inicio del modal dentro del bucle -->
                                            ...
                                            <div class="form-group">
                                                ...
                                                @foreach ($maestrias as $maestria)
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" class="custom-control-input" id="maestria_{{ $maestria->id }}" name="maestrias[]" value="{{ $maestria->id }}" {{ in_array($maestria->id, $secretario->seccion->maestrias->pluck('id')->toArray()) ? 'checked' : '' }}>
                                                        <label class="custom-control-label" for="maestria_{{ $maestria->id }}">{{ $maestria->nombre }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                        ...
                        <!-- Fin del modal dentro del bucle -->
                    @endforeach
                </tbody>
            </table>
            @endif
